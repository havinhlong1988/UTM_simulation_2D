...
model node plus DB/DK/FLZ/RA labels AND major/minor TN candidate flags)
instead of the raw riskmap, and adds two new hard requirements on top of
02's existing FLZ buffer:

  - TN buffer (NEW): every route must pass within a buffer radius of at
    least one TN candidate node -- 300 m if the qualifying TN is major,
    100 m if minor (OR, not AND: whichever type's own radius the route
    actually satisfies). Distance-based like the FLZ buffer -- the route
    does not have to stop at the TN, only pass through its buffer.
    Mirrors the FLZ buffer's replan-via-nearest-node mechanism exactly
    (replan_main_via_tn / backup_via_tn_ring), just with a
    unified major+minor candidate pool sorted by least detour.
  - Turn-angle reduction (NEW): after a route is confirmed FLZ+TN
    compliant, a best-effort post-process (greedy angle-aware waypoint
    removal, then an optional shapely simplify polish) reduces sharp
    turns as much as possible without breaking no-fly validity or either
    buffer requirement. Never a hard cap -- see reduce_route_turn_angles().

Everything else -- main+backup per rank, PAIR_DIRECTION_ORDER interleaved
forward/backward scheduling, lock-as-memory retry-until-exhausted
semantics, backup separation, figure honesty -- is unchanged from 02's v15
engine (see its changelog below, carried over verbatim as the base this
...
Purpose
-------
This script is the master CORRIDOR planning wrapper. It does NOT implement
Theta*. It reads stage 03's whole-map node table (raw model nodes plus
DB/DK/FLZ/RA labels and TN candidate flags), prepares route constraints,
builds the graph, then calls the existing Theta* backend:
...
- FLZ is not a normal endpoint. It is used as emergency-safety attraction
  (soft) AND a hard buffer requirement (every route must pass within
  FLZ_BUFFER_RADIUS_M of at least one FLZ).
- TN (major/minor candidate nodes from stage 03) is a NEW hard buffer
  requirement: every route must pass within TN_BUFFER_MAJOR_RADIUS_M of a
  major TN, or TN_BUFFER_MINOR_RADIUS_M of a minor TN.
...
Important safety fix (carried over from 02)
---------------------------------------------
...
route is rejected and retried in safe A*-delegate mode. The new turn-angle
reduction pass reuses this exact validator to re-check every simplified
shortcut it proposes.
...
----------------------------
Required (from stage 03's master_plan_input_nodes.csv):
    x y slowness label label_prefix is_candidate candidate_type
Optional:
    node_id z is_flz risk_total flz_support emergency_risk
...
-------
    OUTPUT_DIR/
        ...
        route_summary_v1.csv
        all_route_edges_v1.csv
        route_nodes/*.csv
        figures/*.png
        00_all_theta_routes_overview_v1.png
"""
...
def read_node_model(model_file: str | Path) -> pd.DataFrame:
    """Read stage 03's master_plan_input_nodes.csv (comma-separated -- NOT
    02's whitespace-separated raw .xyz). Every raw model node, plus
    DB/DK/FLZ/RA labels and is_candidate/candidate_type TN flags."""
    ...
    required = ["x", "y", "z", "slowness", "label", "label_prefix", "is_candidate", "candidate_type"]
    for col in required:
    ...
    if "is_flz" not in df.columns:
        df["is_flz"] = df["label_prefix"].str.upper() == "FLZ"

    # TN role: a candidate whose own label_prefix is FLZ (or any other
    # objective prefix) is really that objective, not a distinct TN -- see
    # 06's fork notes. Excluding is_flz here mirrors src/routeplan_PSO_ACO.py's
    # build_master_nodes() role priority (DB/DK/FLZ beats TN).
    is_tn = df["is_candidate"].astype(bool) & ~df["is_flz"].astype(bool)
    candidate_type_lower = df["candidate_type"].fillna("").astype(str).str.lower()
    df["tn_role"] = np.where(
        is_tn & (candidate_type_lower == "major"), "major",
        np.where(is_tn & (candidate_type_lower == "minor"), "minor", ""),
    )
...
def _segment_min_distance(df: pd.DataFrame, path_indices: list[int], node_idx_list: list[int]) -> float:
    """Minimum distance from the route polyline to any node in node_idx_list.

    Segment-based, not waypoint-based: smoothed any-angle routes can have
    long straight legs, and a leg can pass right beside a node between two
    waypoints. Checking only waypoints would miss that. Shared by
    route_min_distance_to_flz and route_min_distance_to_tn so the
    point-to-segment math isn't duplicated per node type.
    """
    if not node_idx_list or len(path_indices) == 0:
        return float("inf")

    ref_xy = df.iloc[node_idx_list][_metric_cols(df)].to_numpy(float)
    path_xy = path_to_xy_metric(df, path_indices)
    if len(path_xy) == 1:
        d = np.hypot(ref_xy[:, 0] - path_xy[0, 0], ref_xy[:, 1] - path_xy[0, 1])
    ...
    for k in range(len(path_xy) - 1):
        ...
        denom = float(ab @ ab)
        if denom <= 0.0:
            proj = np.zeros(len(ref_xy))
        else:
            proj = np.clip(((ref_xy - a) @ ab) / denom, 0.0, 1.0)
        closest = a + proj[:, None] * ab
        d = np.hypot(ref_xy[:, 0] - closest[:, 0], ref_xy[:, 1] - closest[:, 1])
...
def route_min_distance_to_flz(df: pd.DataFrame, path_indices: list[int]) -> float:
    ...
    return _segment_min_distance(df, path_indices, flz_node_indices(df))
...
# ======================================================================
# TN buffer requirement (NEW in 06 -- mirrors the FLZ buffer trio above,
# but with a type-dependent radius: major TN nodes get TN_BUFFER_MAJOR_RADIUS_M,
# minor TN nodes get TN_BUFFER_MINOR_RADIUS_M. A route qualifies if it
# passes within EITHER radius of a node of that node's own type -- the
# corridor does not need to route through the TN node itself, only its
# buffer (same "pass nearby, not through" semantics as the FLZ buffer).
# ======================================================================

def tn_node_indices(df: pd.DataFrame) -> tuple[list[int], list[int]]:
    """(major_idx, minor_idx) -- disjoint node index lists, from df['tn_role']
    (derived in read_node_model() from stage 03's is_candidate/candidate_type,
    excluding any candidate that is itself an FLZ/DB/DK node)."""
    role = df["tn_role"].astype(str)
    major = [int(i) for i in np.flatnonzero((role == "major").to_numpy())]
    minor = [int(i) for i in np.flatnonzero((role == "minor").to_numpy())]
    return major, minor


def route_min_distance_to_tn(df: pd.DataFrame, path_indices: list[int]) -> tuple[float, float]:
    """(min_dist_to_major_m, min_dist_to_minor_m), each computed independently
    via the same segment-wise point-to-polyline method as route_min_distance_to_flz."""
    major_idx, minor_idx = tn_node_indices(df)
    return _segment_min_distance(df, path_indices, major_idx), _segment_min_distance(df, path_indices, minor_idx)


def tn_buffer_settings(params: dict[str, Any]) -> tuple[bool, float, float]:
    return (
        bool(pget(params, "TN_BUFFER_ENABLE", False)),
        float(pget(params, "TN_BUFFER_MAJOR_RADIUS_M", 100.0)),
        float(pget(params, "TN_BUFFER_MINOR_RADIUS_M", 50.0)),
    )


def tn_buffer_ok(df: pd.DataFrame, path_indices: list[int], params: dict[str, Any]) -> tuple[bool, float, str]:
    """(ok, best_min_distance_m, matched_role).

    Route qualifies if EITHER a major-within-major_radius OR a
    minor-within-minor_radius holds -- OR, not AND. A single "nearest TN
    overall" float cannot carry the type-dependent threshold correctly:
    the globally nearest TN might be a minor node just outside its own
    tight radius, while a farther major node sits comfortably inside its
    own looser radius. major is preferred for the diagnostic value if
    both qualify; if neither qualifies, returns whichever is
    proportionally closer to passing (distance / its own radius), for a
    more informative reject message.
    """
    tn_on, major_r, minor_r = tn_buffer_settings(params)
    if not tn_on:
        return True, float("nan"), ""
    d_major, d_minor = route_min_distance_to_tn(df, path_indices)
    major_ok = d_major <= major_r
    minor_ok = d_minor <= minor_r
    if major_ok:
        return True, d_major, "major"
    if minor_ok:
        return True, d_minor, "minor"
    ratio_major = d_major / major_r if major_r > 0 else float("inf")
    ratio_minor = d_minor / minor_r if minor_r > 0 else float("inf")
    return (False, d_major, "major") if ratio_major <= ratio_minor else (False, d_minor, "minor")


def tn_buffer_exempt_mask(df: pd.DataFrame, params: dict[str, Any]) -> np.ndarray | None:
    """Nodes inside either TN buffer (major nodes at their own radius,
    minor nodes at their own radius), exempt from lock-overlap accounting.
    Same principle as flz_buffer_exempt_mask. None when the requirement is off.
    """
    tn_on, major_r, minor_r = tn_buffer_settings(params)
    if not tn_on:
        return None
    ...
    for i in major_idx:
        fx, fy = float(mxy[i, 0]), float(mxy[i, 1])
        mask |= np.hypot(xs - fx, ys - fy) <= float(major_r)
    for i in minor_idx:
        fx, fy = float(mxy[i, 0]), float(mxy[i, 1])
        mask |= np.hypot(xs - fx, ys - fy) <= float(minor_r)
    return mask


def replan_main_via_tn(
    df: pd.DataFrame,
    cell_to_idx: dict[tuple[int, int], int],
    base_allowed: np.ndarray,
    start_idx: int,
    ...
    """Force a route through a TN buffer by planning start->TN->end.

    Mirrors replan_main_via_flz exactly. Candidate pool = major + minor TN
    nodes UNIFIED (not tried major-then-minor in tiers -- a nearby minor
    is often a smaller detour than a distant major, and tn_buffer_ok()
    doesn't care which type actually satisfies the requirement), sorted by
    ascending straight-line detour dist(start,tn)+dist(tn,end) -- same
    least-detour-first philosophy as the FLZ version. The merged route
    must still pass the lock-overlap test like any other main.
    """
    major_idx, minor_idx = tn_node_indices(df)
    tn_idx = major_idx + minor_idx
    if not tn_idx:
        return {"success": False, "message": "TN buffer required but the model has no TN candidate nodes.", "path_indices": []}

    mxy = all_metric_xy(df)
    sx, sy = float(mxy[start_idx, 0]), float(mxy[start_idx, 1])
    ex, ey = float(mxy[end_idx, 0]), float(mxy[end_idx, 1])
    txy = mxy[tn_idx]
    detour = np.hypot(txy[:, 0] - sx, txy[:, 1] - sy) + np.hypot(txy[:, 0] - ex, txy[:, 1] - ey)
    order = [tn_idx[i] for i in np.argsort(detour)]
    ...
    if _flz_exempt is not None:
        endpoint_mask = endpoint_mask | _flz_exempt
    _tn_exempt = tn_buffer_exempt_mask(df, params)
    if _tn_exempt is not None:
        endpoint_mask = endpoint_mask | _tn_exempt
    locked_mask = build_locked_node_mask(df, locked_paths, endpoint_mask, lock_radius_m)

    last_message = "No TN candidate produced a valid via-route."
    for tn_i in order:
        tn_label = str(df.iloc[tn_i]["candidate_id"]) or str(df.iloc[tn_i]["node_id"])
        tn_role = str(df.iloc[tn_i]["tn_role"])
        leg1 = run_theta_once(
            base_model=df, cell_to_idx=cell_to_idx, mask=base_allowed,
            start_idx=start_idx, end_idx=int(tn_i), params=params,
            route_name=f"{route_name}_via_{tn_label}_leg1", safe_fallback=True,
        )
        if not leg1.get("success", False):
            last_message = f"Leg to {tn_label} failed: {leg1.get('message', '')}"
            continue
        leg2 = run_theta_once(
            base_model=df, cell_to_idx=cell_to_idx, mask=base_allowed,
            start_idx=int(tn_i), end_idx=end_idx, params=params,
            route_name=f"{route_name}_via_{tn_label}_leg2", safe_fallback=True,
        )
        if not leg2.get("success", False):
            last_message = f"Leg from {tn_label} failed: {leg2.get('message', '')}"
            continue
        ...
        result: dict[str, Any] = {
            "success": True,
            "message": f"Route forced through TN buffer via {tn_role} {tn_label}.",
            "route_name": route_name,
            "path_indices": merged,
            "distance_m": path_distance_m(df, merged),
            "total_cost": float(leg1.get("total_cost", np.nan)) + float(leg2.get("total_cost", np.nan)),
            ...
            "expanded_nodes": int(leg1.get("expanded_nodes", 0) or 0) + int(leg2.get("expanded_nodes", 0) or 0),
            "safe_fallback": bool(leg1.get("safe_fallback", False) or leg2.get("safe_fallback", False)),
            "tn_buffer_via": tn_label,
            "tn_buffer_matched_role": tn_role,
            "mean_flz_support": path_mean_column(df, merged, "flz_support"),
            "mean_emergency_risk": path_mean_column(df, merged, "emergency_risk"),
            "mean_other_terminal_avoidance": path_mean_column(df, merged, "other_terminal_avoidance"),
            "mean_traffic_penalty_factor": path_mean_column(df, merged, "traffic_penalty_factor"),
        ...
        if lock_enabled and np.any(locked_mask):
            ok_overlap, result = route_respects_lock_limit(result, locked_mask, endpoint_mask, max_overlap_pct)
            if not ok_overlap:
                last_message = f"Via-{tn_label} route exceeded the lock overlap limit."
                continue
        return result
...
def backup_via_tn_ring(
    ...
    block_mask: np.ndarray | None,
    main_path: list[int],
    tn_major_radius: float,
    tn_minor_radius: float,
) -> dict[str, Any]:
    """Force a backup through the TN buffer under the same corridor/block
    masks as a normal backup. Mirrors backup_via_flz_ring; the in_buffer
    mask is the OR of (major nodes @ their own radius) and (minor nodes @
    their own radius). Ring candidates are ordinary allowed nodes inside
    that buffer -- not necessarily TN nodes themselves, same as the FLZ
    version -- ranked nearest-to-nearest-TN first.
    """
    major_idx, minor_idx = tn_node_indices(df)
    tn_idx = major_idx + minor_idx
    if not tn_idx:
        return {"success": False, "message": "No TN candidate nodes.", "path_indices": []}

    mxy = all_metric_xy(df)
    xs, ys = mxy[:, 0], mxy[:, 1]
    in_buffer = np.zeros(len(df), dtype=bool)
    dist_to_tn = np.full(len(df), np.inf)
    for i in major_idx:
        fx, fy = float(mxy[i, 0]), float(mxy[i, 1])
        d = np.hypot(xs - fx, ys - fy)
        in_buffer |= d <= float(tn_major_radius)
        dist_to_tn = np.minimum(dist_to_tn, d)
    for i in minor_idx:
        fx, fy = float(mxy[i, 0]), float(mxy[i, 1])
        d = np.hypot(xs - fx, ys - fy)
        in_buffer |= d <= float(tn_minor_radius)
        dist_to_tn = np.minimum(dist_to_tn, d)
    ...
    cand_idx = np.flatnonzero(cand)
    if len(cand_idx) == 0:
        return {"success": False, "message": "No allowed node inside TN buffer, corridor, and off-main block.", "path_indices": []}
    cand_idx = cand_idx[np.argsort(dist_to_tn[cand_idx])][:6]

    last_message = "All TN ring candidates failed."
    for ci in cand_idx:
        ci = int(ci)
        leg1 = run_project_theta(
            base_model=df, cell_to_idx=cell_to_idx, base_allowed_mask=base_allowed,
        ...
        p2 = [int(v) for v in leg2.get("path_indices", [])]
        merged = p1 + p2[1:]
        min_d_major, min_d_minor = route_min_distance_to_tn(df, merged)
        ring_role = str(df.iloc[ci]["tn_role"]) or ("major" if min_d_major <= tn_major_radius else "minor")
        ring_label = str(df.iloc[ci]["candidate_id"]) or str(df.iloc[ci]["node_id"])
        return {
            "success": True,
            "message": f"Backup forced through TN buffer via ring node {ring_label}.",
            "route_name": route_name,
            "path_indices": merged,
            "distance_m": path_distance_m(df, merged),
            "total_cost": float(leg1.get("total_cost", np.nan)) + float(leg2.get("total_cost", np.nan)),
            ...
            "expanded_nodes": int(leg1.get("expanded_nodes", 0) or 0) + int(leg2.get("expanded_nodes", 0) or 0),
            "safe_fallback": bool(leg1.get("safe_fallback", False) or leg2.get("safe_fallback", False)),
            "tn_buffer_via": ring_label,
            "tn_buffer_matched_role": ring_role,
            "tn_buffer_min_distance_m": min_d_major if ring_role == "major" else min_d_minor,
            "tn_buffer_ok": bool(min_d_major <= tn_major_radius or min_d_minor <= tn_minor_radius),
        }
...
# ======================================================================
# expand_and_validate_path_by_grid_los() above always replaces a route's
# path_indices with its DENSE, Bresenham-expanded grid path (every cell
# along each shortcut segment) -- confirmed by tracing how run_project_theta
# uses it (result["path_indices"] = expanded_path once bad_segments is
# empty). Computing turn angles directly on that dense path would mostly
# measure single-grid-cell staircase jitter, not real corridor bends, so
# reduce_route_turn_angles() first SIMPLIFIES down to the route's actual
# corner points (shapely, tolerance tuned to absorb that jitter), then
# greedily prunes remaining small-angle corners on the sparse result, then
# re-expands the surviving corners back into one dense, grid-validated
# path -- same representation convention as every other route here.

def compute_turn_angles_deg(df: pd.DataFrame, path_indices: list[int]) -> np.ndarray:
    """Turn angle in degrees at each interior waypoint (len = n-2), via
    arctan2 of consecutive direction vectors in metric xy. 0 deg =
    straight through, 180 deg = full reversal. Meaningful on a
    SPARSE/simplified polyline -- see module note above."""
    if len(path_indices) < 3:
        return np.zeros(0, dtype=float)
    xy = path_to_xy_metric(df, path_indices)
    v1 = xy[1:-1] - xy[:-2]
    v2 = xy[2:] - xy[1:-1]
    ang1 = np.arctan2(v1[:, 1], v1[:, 0])
    ang2 = np.arctan2(v2[:, 1], v2[:, 0])
    diff = np.degrees(np.abs((ang2 - ang1 + np.pi) % (2 * np.pi) - np.pi))
    return diff


def _nearest_path_node_to_xy(df: pd.DataFrame, path_indices: list[int], xy_point: np.ndarray) -> int:
    """Snap an arbitrary xy point (e.g. from shapely simplify) back to the
    nearest node already present in path_indices -- this is a nodal grid
    model, a simplified polyline must stay on real, already-validated nodes."""
    xy = path_to_xy_metric(df, path_indices)
    d = np.hypot(xy[:, 0] - xy_point[0], xy[:, 1] - xy_point[1])
    return int(path_indices[int(np.argmin(d))])


def _stitch_dense_path(
    df: pd.DataFrame,
    cell_to_idx: dict[tuple[int, int], int],
    allowed_mask: np.ndarray,
    start_idx: int,
    ...
    """Expand+validate a sparse corner list into one dense, grid-validated
    node-index path via expand_and_validate_path_by_grid_los() on each
    consecutive pair. None if any segment crosses a blocked cell."""
    if len(corners) < 2:
        return list(corners)
    dense: list[int] = []
    for a, b in zip(corners[:-1], corners[1:]):
        expanded, bad = expand_and_validate_path_by_grid_los(
            df=df, cell_to_idx=cell_to_idx, path_indices=[a, b],
            allowed_mask=allowed_mask, start_idx=start_idx, end_idx=end_idx,
        )
        if bad:
            return None
        if dense and expanded and expanded[0] == dense[-1]:
            dense.extend(expanded[1:])
        else:
            dense.extend(expanded)
    return dense


TURN_REDUCTION_STAT_KEYS = (
    "n_waypoints_before", "n_waypoints_after",
    "max_turn_angle_deg_before", "max_turn_angle_deg_after",
    "mean_turn_angle_deg_before", "mean_turn_angle_deg_after",
    "turn_reduction_applied",
)


def reduce_route_turn_angles(
    df: pd.DataFrame,
    cell_to_idx: dict[tuple[int, int], int],
    path_indices: list[int],
    allowed_mask: np.ndarray,
    ...
    """Best-effort post-process on an already-accepted, already-validated
    route: reduce sharp/unnecessary turns as much as possible without ever
    breaking no-fly validity or the FLZ/TN buffer requirements. Never a
    hard cap -- if nothing can be safely removed, the original path is
    returned unchanged.

    1. Simplify the dense path down to its real corners (shapely).
    2. Greedy pass: repeatedly try removing the smallest-remaining-angle
       corner (skip corners already below TURN_REDUCTION_MIN_ANGLE_DEG --
       not worth the iteration); accept the removal only if the
       resulting corner list re-stitches into a valid dense path AND
       still satisfies the FLZ buffer (if enabled) AND the TN buffer (if
       enabled). Repeat until nothing removable or
       TURN_REDUCTION_MAX_ITERATIONS.
    3. Re-densify the surviving corners into the final validated path.
    """
    n_before = len(path_indices)
    empty_stats = {
        "path_indices": list(path_indices),
        "n_waypoints_before": n_before, "n_waypoints_after": n_before,
        "max_turn_angle_deg_before": np.nan, "max_turn_angle_deg_after": np.nan,
        "mean_turn_angle_deg_before": np.nan, "mean_turn_angle_deg_after": np.nan,
        "turn_reduction_applied": False,
    }
    if not bool(pget(params, "TURN_REDUCTION_ENABLE", True)) or n_before < 3:
        return empty_stats

    tolerance_m = float(pget(params, "TURN_REDUCTION_SIMPLIFY_TOLERANCE_M", 25.0))
    xy = path_to_xy_metric(df, path_indices)
    simplified_xy = np.asarray(LineString(xy).simplify(tolerance_m, preserve_topology=True).coords)

    corners = [int(path_indices[0])]
    for pt in simplified_xy[1:-1]:
        node = _nearest_path_node_to_xy(df, path_indices, pt)
        if node != corners[-1]:
            corners.append(node)
    if corners[-1] != int(path_indices[-1]):
        corners.append(int(path_indices[-1]))
    corners = [c for i, c in enumerate(corners) if i == 0 or c != corners[i - 1]]

    angles_before = compute_turn_angles_deg(df, corners)
    max_before = float(np.max(angles_before)) if len(angles_before) else np.nan
    mean_before = float(np.mean(angles_before)) if len(angles_before) else np.nan

    flz_on, flz_radius = flz_buffer_settings(params)
    min_angle = float(pget(params, "TURN_REDUCTION_MIN_ANGLE_DEG", 5.0))
    max_iter = int(pget(params, "TURN_REDUCTION_MAX_ITERATIONS", 200))

    eps = 1.0e-6
    for _ in range(max_iter):
        if len(corners) < 3:
            break
        angles = compute_turn_angles_deg(df, corners)
        if len(angles) == 0:
            break
        cur_max = float(np.max(angles))
        cur_mean = float(np.mean(angles))
        removed = False
        for j in np.argsort(angles):
            if angles[j] < min_angle:
                continue
            i_pos = int(j) + 1
            a, k = corners[i_pos - 1], corners[i_pos + 1]
            trial_corners = corners[:i_pos] + corners[i_pos + 1:]
            # Removing a waypoint changes the angle at its surviving
            # neighbors too -- the locally smallest-angle point isn't
            # guaranteed to be safe to drop, since the new direct segment
            # can make a NEIGHBORING corner sharper than it was. Only
            # accept a removal that doesn't make the overall max/mean
            # angle worse, so the pass is monotonically non-regressing.
            trial_angles = compute_turn_angles_deg(df, trial_corners)
            trial_max = float(np.max(trial_angles)) if len(trial_angles) else 0.0
            trial_mean = float(np.mean(trial_angles)) if len(trial_angles) else 0.0
            if trial_max > cur_max + eps or trial_mean > cur_mean + eps:
                continue
            trial_full = _stitch_dense_path(df, cell_to_idx, allowed_mask, start_idx, end_idx, trial_corners)
            if trial_full is None:
                continue
            if flz_on and route_min_distance_to_flz(df, trial_full) > flz_radius:
                continue
            if not tn_buffer_ok(df, trial_full, params)[0]:
                continue
            corners = trial_corners
            removed = True
            break
        if not removed:
            break

    angles_after = compute_turn_angles_deg(df, corners)
    max_after = float(np.max(angles_after)) if len(angles_after) else np.nan
    mean_after = float(np.mean(angles_after)) if len(angles_after) else np.nan

    final_path = _stitch_dense_path(df, cell_to_idx, allowed_mask, start_idx, end_idx, corners)
    if final_path is None:
        # Every accepted trial above was re-validated, so this shouldn't
        # happen -- but never regress below the original validated path.
        final_path = list(path_indices)

    return {
        "path_indices": final_path,
        "n_waypoints_before": n_before, "n_waypoints_after": len(final_path),
        "max_turn_angle_deg_before": max_before, "max_turn_angle_deg_after": max_after,
        "mean_turn_angle_deg_before": mean_before, "mean_turn_angle_deg_after": mean_after,
        "turn_reduction_applied": final_path != list(path_indices),
    }
...
# ======================================================================
# Within-pair locking above keeps a pair's own main/backup routes from
# duplicating each other. It says nothing about DIFFERENT DB<->DK pairs --
# each pair is otherwise planned completely independently, so two pairs'
# corridors could heavily overlap outside the shared TN/FLZ backbone. This
# reuses the exact same generic lock-mask/overlap-stats machinery above,
# just against a cross-pair "already accepted by an earlier pair" path
# list, with its own threshold and its own FLZ+TN buffer exemption
# (sharing the backbone there is the intended design, not something to
# penalize).

def cross_pair_overlap_settings(params: dict[str, Any]) -> tuple[bool, float]:
    return (
        bool(pget(params, "CROSS_PAIR_OVERLAP_ENABLE", False)),
        float(pget(params, "CROSS_PAIR_MAX_ROUTE_NODE_OVERLAP_PERCENT", 20.0)),
    )


def cross_pair_exempt_mask(df: pd.DataFrame, params: dict[str, Any]) -> np.ndarray:
    """FLZ + TN buffer zones, exempt from cross-pair overlap accounting."""
    mask = np.zeros(len(df), dtype=bool)
    _flz = flz_buffer_exempt_mask(df, params)
    if _flz is not None:
        mask |= _flz
    _tn = tn_buffer_exempt_mask(df, params)
    if _tn is not None:
        mask |= _tn
    return mask


def cross_pair_overlap_ok(
    df: pd.DataFrame,
    path_indices: list[int],
    cross_pair_locked_paths: list[list[int]],
    exempt_mask: np.ndarray,
    lock_radius_m: float,
    max_pct: float,
) -> tuple[bool, dict[str, Any]]:
    """Check path_indices against OTHER pairs' already-accepted routes."""
    if not cross_pair_locked_paths:
        return True, {
            "cross_pair_overlap_ok": True,
            "cross_pair_overlap_nodes": 0,
            "cross_pair_overlap_checked_nodes": 0,
            "cross_pair_overlap_percent": 0.0,
        }
    locked_mask = build_locked_node_mask(df, cross_pair_locked_paths, exempt_mask, lock_radius_m)
    overlap_nodes, checked_nodes, overlap_pct = route_overlap_stats(path_indices, locked_mask, exempt_mask)
    ok = overlap_pct <= float(max_pct) + 1.0e-9
    return ok, {
        "cross_pair_overlap_ok": bool(ok),
        "cross_pair_overlap_nodes": int(overlap_nodes),
        "cross_pair_overlap_checked_nodes": int(checked_nodes),
        "cross_pair_overlap_percent": float(overlap_pct),
    }
...
def generate_direction_route_sets(
    ...
    cross_pair_locked_paths: list[list[int]] | None = None,
    ...
    # Cross-pair overlap (NEW in 06): read-only avoid-list of OTHER pairs'
    # already-accepted routes, with its own threshold and its own FLZ+TN
    # buffer exemption (computed once per call, reused for every attempt).
    cp_on, cp_max_pct = cross_pair_overlap_settings(params)
    cp_exempt = cross_pair_exempt_mask(df, params) if cp_on else None
    _, _, cp_lock_radius_m, _ = lock_settings(params)
    ...
    for rank in rank_list:
        ...
        while True:
            ...
            # ---- TN buffer enforcement (NEW in 06, mirrors the FLZ block
            # above) ----
            tn_on, tn_major_r, tn_minor_r = tn_buffer_settings(params)
            if tn_on:
                tn_ok, tn_min_d, tn_role = tn_buffer_ok(df, main_path, params)
                main_result["tn_buffer_min_distance_m"] = float(tn_min_d)
                main_result["tn_buffer_matched_role"] = tn_role
                main_result["tn_buffer_ok"] = bool(tn_ok)
                if not tn_ok:
                    via_result = replan_main_via_tn(
                        df=df, cell_to_idx=cell_to_idx, base_allowed=base_allowed,
                        start_idx=start_idx, end_idx=end_idx, params=params,
                        route_name=attempt_main_name, locked_paths=locked_paths + extra_locked,
                    )
                    ...
                    if via_result.get("success", False):
                        via_path = [int(v) for v in via_result.get("path_indices", [])]
                        via_tn_ok, via_tn_d, via_tn_role = tn_buffer_ok(df, via_path, params)
                        via_result["tn_buffer_min_distance_m"] = float(via_tn_d)
                        via_result["tn_buffer_matched_role"] = via_tn_role
                        via_result["tn_buffer_ok"] = bool(via_tn_ok)
                        # via-TN is a brand-new path (replan_main_via_tn
                        # doesn't check FLZ) -- re-verify it still satisfies
                        # the FLZ buffer too, since the FLZ check above only
                        # validated the ORIGINAL main_path, not this one.
                        via_flz_ok = True
                        if flz_on:
                            via_flz_d = route_min_distance_to_flz(df, via_path)
                            via_result["flz_buffer_min_distance_m"] = float(via_flz_d)
                            via_flz_ok = bool(via_flz_d <= flz_radius)
                            via_result["flz_buffer_ok"] = via_flz_ok
                        via_ok = bool(via_tn_ok and via_flz_ok)
                    if via_ok:
                        main_result = via_result
                        main_path = via_path
                    else:
                        ...
                        cap_note = f"/{max_retry}" if max_retry > 0 else ""
                        print(
                            f"  reject: {attempt_main_name} misses TN buffer "
                            f"(min dist {tn_min_d:.1f} m) and via-TN replan failed or broke FLZ "
                            f"(attempt {attempt}{cap_note}) -- locking it, searching for another main"
                        )
                        continue

            # ---- Cross-pair overlap enforcement (NEW in 06) ----
            if cp_on and cross_pair_locked_paths:
                cp_ok, cp_stats = cross_pair_overlap_ok(
                    df, main_path, cross_pair_locked_paths, cp_exempt, cp_lock_radius_m, cp_max_pct,
                )
                main_result.update(cp_stats)
                if not cp_ok:
                    previous_main_paths.append(main_path)
                    locked_paths.append(main_path)
                    rejected_count += 1
                    cap_note = f"/{max_retry}" if max_retry > 0 else ""
                    print(
                        f"  reject: {attempt_main_name} overlaps other pairs' corridors too much "
                        f"({cp_stats['cross_pair_overlap_percent']:.1f}% > {cp_max_pct:.1f}%) "
                        f"(attempt {attempt}{cap_note}) -- locking it, searching for another main"
                    )
                    continue

            use_soft_parallel = bool(pget(params, "PARALLEL_SOFT_BUFFER_ENABLE", False))
            if use_soft_parallel:
                # Dual-parallel soft-buffer mode: path2 is planned by the
                # outer penalty loop around the accepted main, replacing
                # the corridor+block-mask backup search.
                backup_result = make_parallel_route_soft_buffer(
                    df=df,
                    cell_to_idx=cell_to_idx,
                    base_allowed=base_allowed,
                    start_idx=start_idx,
            ...
            # ---- Cross-pair overlap enforcement on the backup (NEW in 06) ----
            if backup_ok and cp_on and cross_pair_locked_paths:
                backup_path_for_cp = [int(v) for v in backup_result.get("path_indices", [])]
                cp_ok_b, cp_stats_b = cross_pair_overlap_ok(
                    df, backup_path_for_cp, cross_pair_locked_paths, cp_exempt, cp_lock_radius_m, cp_max_pct,
                )
                backup_result.update(cp_stats_b)
                if not cp_ok_b:
                    backup_ok = False
                    backup_result["success"] = False
                    backup_result["message"] = (
                        str(backup_result.get("message", ""))
                        + f" Backup overlaps other pairs' corridors too much "
                        + f"({cp_stats_b['cross_pair_overlap_percent']:.1f}% > {cp_max_pct:.1f}%)."
                    ).strip()
            ...
            if backup_ok:
                # Turn-angle reduction (NEW in 06): applied to the FINAL
                # accepted geometry, right before it's locked/recorded.
                # main_path was already locked above (unconditionally,
                # before backup success was known -- 02's existing
                # "whichever main was tried goes on the avoid-list now"
                # design) using its PRE-reduction shape. That's a safe,
                # slightly conservative choice -- turn reduction only ever
                # removes/straightens waypoints, never adds new ones, so
                # the lock mask stays a (mildly over-inclusive) superset of
                # the true final footprint; future ranks' overlap checks
                # stay valid. The backup has not been locked yet at this
                # point, so it locks its accurate, final, reduced geometry.
                backup_path = [int(v) for v in backup_result.get("path_indices", [])]

                main_reduction = reduce_route_turn_angles(
                    df=df, cell_to_idx=cell_to_idx, path_indices=main_path,
                    allowed_mask=base_allowed, start_idx=start_idx, end_idx=end_idx, params=params,
                )
                main_result["path_indices"] = main_reduction["path_indices"]
                main_result["distance_m"] = path_distance_m(df, main_reduction["path_indices"])
                main_result.update({k: main_reduction[k] for k in TURN_REDUCTION_STAT_KEYS})

                if use_soft_parallel:
                    # Turn reduction straightens waypoints toward the direct
                    # line -- i.e. back toward the main -- and would undo the
                    # soft-buffer separation the penalty loop just paid for.
                    # Keep the parallel's geometry exactly as planned.
                    backup_result["turn_reduction_applied"] = False
                    backup_result["n_waypoints_before"] = len(backup_path)
                    backup_result["n_waypoints_after"] = len(backup_path)
                else:
                    backup_reduction = reduce_route_turn_angles(
                        df=df, cell_to_idx=cell_to_idx, path_indices=backup_path,
                        allowed_mask=base_allowed, start_idx=start_idx, end_idx=end_idx, params=params,
                    )
                    backup_result["path_indices"] = backup_reduction["path_indices"]
                    backup_result["distance_m"] = path_distance_m(df, backup_reduction["path_indices"])
                    backup_result.update({k: backup_reduction[k] for k in TURN_REDUCTION_STAT_KEYS})

                locked_paths.append(list(backup_result["path_indices"]))
...
def make_backup_route(
    ...
    _tn_exempt = tn_buffer_exempt_mask(df, params)
    overlap_exempt_mask = (overlap_exempt_mask | _tn_exempt) if _tn_exempt is not None else overlap_exempt_mask
        ...
        for lock_mode, this_block_mask in candidate_attempts:
            ...
            if result.get("success", False):
                ...
                if ok_overlap:
                    ...
                    # ---- TN buffer enforcement (NEW in 06, mirrors the
                    # FLZ ring block above) ----
                    tn_on, tn_major_r, tn_minor_r = tn_buffer_settings(params)
                    if tn_on:
                        cur_path = [int(v) for v in result.get("path_indices", [])]
                        tn_ok, tn_min_d, tn_role = tn_buffer_ok(df, cur_path, params)
                        result["tn_buffer_min_distance_m"] = float(tn_min_d)
                        result["tn_buffer_matched_role"] = tn_role
                        result["tn_buffer_ok"] = bool(tn_ok)
                        if not tn_ok:
                            # Same reasoning as the FLZ ring above: a
                            # cost-minimizing backup will never take the
                            # main's dip toward a TN voluntarily.
                            via_backup_tn = backup_via_tn_ring(
                                df=df, cell_to_idx=cell_to_idx, base_allowed=base_allowed,
                                start_idx=start_idx, end_idx=end_idx, params=params,
                                route_name=route_name, corridor_mask=corridor_mask,
                                block_mask=this_block_mask, main_path=main_path,
                                tn_major_radius=tn_major_r, tn_minor_radius=tn_minor_r,
                            )
                            if via_backup_tn.get("success", False):
                                ok3, via_backup_tn = route_respects_lock_limit(via_backup_tn, locked_mask, overlap_exempt_mask, max_overlap_pct)
                                if ok3:
                                    # via-TN is a brand-new path; re-verify FLZ too.
                                    via_flz_ok3 = True
                                    if flz_on:
                                        via_flz_d3 = route_min_distance_to_flz(df, [int(v) for v in via_backup_tn.get("path_indices", [])])
                                        via_backup_tn["flz_buffer_min_distance_m"] = float(via_flz_d3)
                                        via_flz_ok3 = bool(via_flz_d3 <= flz_radius)
                                        via_backup_tn["flz_buffer_ok"] = via_flz_ok3
                                    if via_flz_ok3:
                                        sep_ok3, sep_stats3 = backup_separation_verdict(
                                            df, main_path, [int(v) for v in via_backup_tn.get("path_indices", [])],
                                            start_idx, end_idx, params,
                                        )
                                        via_backup_tn.update(sep_stats3)
                                        if sep_ok3:
                                            via_backup_tn["backup_corridor_m"] = float(corridor_m)
                                            via_backup_tn["backup_block_main_radius_m"] = block_radius
                                            via_backup_tn["lock_mode"] = lock_mode
                                            via_backup_tn["duplicated_from_main"] = False
                                            via_backup_tn.update(backup_vs_main_geometry(df, main_path, via_backup_tn.get("path_indices", [])))
                                            return via_backup_tn
                            result["success"] = False
                            result["message"] = (
                                str(result.get("message", ""))
                                + f" Backup misses TN buffer (min {tn_min_d:.1f} m) and via-ring replan failed."
                            ).strip()
                            best_failed = result
                            continue
...
# ======================================================================
#
# Implements dual_parallel_route_buffer_design.md: instead of the legacy
# corridor+block-mask backup search, the parallel route (path2) is found
# by an OUTER PENALTY LOOP around the unchanged Theta* solver. path1
# (the accepted main) acts as the fixed reference; path2 is re-searched
# with a repulsive cost field laid around path1:
#
#   penalty(node) = lambda * ((D - d) / D)^2      for d < D = BUFFER_TARGET_M
#                   * BUFFER_CROSSING_MULTIPLIER  near detected crossings
#
# The buffer is SOFT: >= BUFFER_TARGET_M separation is preferred, narrow
# spots at geometric bottlenecks are tolerated, and crossings (d -> 0 /
# side change) are penalized much harder than mere narrowness. lambda
# escalates by BUFFER_PENALTY_GROWTH_RATE whenever re-searching stops
# improving, up to BUFFER_PENALTY_LAMBDA_CAP; the best-scoring path2
# seen so far is returned together with a violation report (there is no
# hard failure -- path2 is always a valid obstacle-avoiding route, just
# possibly closer to path1 than desired in places).
#
# thetastar.py itself is untouched: the field is injected by adding the
# penalty onto flyable-node slowness (clamped below the no-fly
# threshold), exactly the same mechanism apply_pair_terminal_traffic_
# avoidance() already uses for terminal avoidance.

def _route_xy(df: pd.DataFrame, path_indices: list[int]) -> np.ndarray:
    if not path_indices:
        return np.empty((0, 2), dtype=float)
    return df.iloc[[int(i) for i in path_indices]][["x", "y"]].to_numpy(float)


def _resample_polyline_m(xy: np.ndarray, step_m: float) -> np.ndarray:
    """Points spaced ~step_m along the polyline arc-length (endpoints kept)."""
    if len(xy) < 2:
        return xy.copy()
    ...
    if total <= 1.0e-9:
        return xy[:1].copy()
    n = max(int(math.ceil(total / max(float(step_m), 1.0e-6))) + 1, 2)
...
def _min_distance_to_polyline(points: np.ndarray, poly: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """(min_dist, nearest_point) from each point to a polyline (perpendicular projection)."""
    if len(poly) == 0 or len(points) == 0:
        return np.full(len(points), np.inf), np.zeros((len(points), 2))
    if len(poly) == 1:
        d = np.hypot(points[:, 0] - poly[0, 0], points[:, 1] - poly[0, 1])
        return d, np.repeat(poly[0][None, :], len(points), axis=0)
    a = poly[:-1]
    ab = poly[1:] - a
    ab2 = np.maximum((ab ** 2).sum(axis=1), 1.0e-12)
    ap = points[:, None, :] - a[None, :, :]
    t = np.clip((ap * ab[None, :, :]).sum(axis=2) / ab2[None, :], 0.0, 1.0)
    proj = a[None, :, :] + t[:, :, None] * ab[None, :, :]
    d = np.hypot(points[:, None, 0] - proj[:, :, 0], points[:, None, 1] - proj[:, :, 1])
    j = d.argmin(axis=1)
    rows = np.arange(len(points))
    return d[rows, j], proj[rows, j]


def compute_clearance_profile(ref_xy: np.ndarray, other_xy: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Continuous clearance of path2 measured from resampled path1 samples.

    Returns (sample_xy, dist, side): distance to the nearest point on
    path2 and which side of path1 that nearest point lies on (+1/-1,
    0 = coincident) -- side flips reveal crossings.
    """
    dist, nearest = _min_distance_to_polyline(ref_xy, other_xy)
    # Tangent along path1 at each sample (central differences).
    tang = np.gradient(ref_xy, axis=0)
    to_other = nearest - ref_xy
    cross_z = tang[:, 0] * to_other[:, 1] - tang[:, 1] * to_other[:, 0]
    side = np.sign(cross_z)
    side[dist <= 1.0e-6] = 0.0
    return ref_xy, dist, side


def detect_crossing_points(sample_xy: np.ndarray, dist: np.ndarray, side: np.ndarray) -> np.ndarray:
    """Sample locations where path2 actually touches or switches sides of path1."""
    pts: list[np.ndarray] = []
    prev_sign = 0.0
    for i in range(len(side)):
        s = float(side[i])
        if float(dist[i]) <= 1.0:  # d has genuinely reached 0 (grid tolerance)
            pts.append(sample_xy[i])
            prev_sign = 0.0
            continue
        if s == 0.0:
            continue
        if prev_sign != 0.0 and s != prev_sign:
            pts.append(0.5 * (sample_xy[i - 1] + sample_xy[i]))
        prev_sign = s
    if not pts:
        return np.empty((0, 2), dtype=float)
...
def parallel_violation_score(dist: np.ndarray, n_crossings: int, params: dict[str, Any]) -> float:
    """Crossings always dominate plain narrowness -- 'minimize crossings'
    outranks 'keep the full 50 m' by design."""
    target = float(pget(params, "BUFFER_TARGET_M", 50.0))
    mult = float(pget(params, "BUFFER_CROSSING_MULTIPLIER", 8.0))
    narrow_cost = float(np.maximum(target - dist, 0.0).sum())
    crossing_cost = float(n_crossings) * mult * target
    return narrow_cost + crossing_cost


def build_repulsive_slowness_model(
    df: pd.DataFrame,
    ref_xy: np.ndarray,
    lam: float,
    crossing_pts: np.ndarray,
    params: dict[str, Any],
) -> pd.DataFrame:
    """Model copy whose flyable slowness carries the repulsive field around path1."""
    target = float(pget(params, "BUFFER_TARGET_M", 50.0))
    field_radius = float(pget(params, "BUFFER_FIELD_RADIUS_M", 75.0))
    mult = float(pget(params, "BUFFER_CROSSING_MULTIPLIER", 8.0))
    ...
    try:
        from scipy.spatial import cKDTree
        d_ref, _ = cKDTree(ref_xy).query(xy, k=1)
    except Exception:
        d_ref, _ = _min_distance_to_polyline(xy, ref_xy)

    penalty = np.zeros(len(df), dtype=float)
    near = (d_ref < min(target, field_radius))
    if near.any():
        penalty[near] = float(lam) * ((target - d_ref[near]) / target) ** 2
        if crossing_pts is not None and len(crossing_pts):
            try:
                from scipy.spatial import cKDTree
                d_cross, _ = cKDTree(crossing_pts).query(xy, k=1)
            except Exception:
                d_cross = np.min(
                    np.hypot(xy[:, 0:1] - crossing_pts[None, :, 0], xy[:, 1:2] - crossing_pts[None, :, 1]),
                    axis=1,
                )
            boost = near & (d_cross <= target)
            penalty[boost] *= mult

    work = df.copy()
    slow = work["slowness"].to_numpy(float)
    flyable = slow < nofly_thr
    new_slow = slow.copy()
    # Clamp strictly below the no-fly threshold: the field must make nodes
    # expensive, never turn them into obstacles.
    new_slow[flyable] = np.minimum(slow[flyable] + penalty[flyable], 0.95 * nofly_thr)
    work["slowness"] = new_slow
    return work


def make_parallel_route_soft_buffer(
    df: pd.DataFrame,
    cell_to_idx: dict[tuple[int, int], int],
    base_allowed: np.ndarray,
    start_idx: int,
    ...
    """Outer penalty loop: plan path2 parallel to the accepted main (path1)."""
    target = float(pget(params, "BUFFER_TARGET_M", 50.0))
    step_m = float(pget(params, "BUFFER_SAMPLE_STEP_M", 10.0))
    lam = float(pget(params, "BUFFER_PENALTY_LAMBDA_INIT", 1.0))
    growth = float(pget(params, "BUFFER_PENALTY_GROWTH_RATE", 1.5))
    lam_cap = float(pget(params, "BUFFER_PENALTY_LAMBDA_CAP", 50.0))
    max_iter = int(pget(params, "BUFFER_MAX_ITERATIONS", 20))

    ref_xy = _resample_polyline_m(_route_xy(df, main_path), step_m)
    # Dense (untrimmed) centerline: kept for the outer corridor below.
    # distance_to_path() only measures to path WAYPOINTS, which breaks
    # for sparse mains (e.g. a straight 2-waypoint network leg) -- the
    # resampled polyline is correct for any waypoint density.
    ref_xy_full = ref_xy

    # Both routes share their start/end terminals, so d -> 0 there is
    # unavoidable by design: trim the endpoint driveways (same exemption
    # the legacy planner's block carve uses) from the reference samples.
    # Otherwise the endpoints register as permanent crossings/violations
    # that no amount of lambda escalation can ever fix, and the field
    # would push path2 away from terminals it MUST converge to.
    endpoint_buffer_m = float(pget(params, "ENDPOINT_BUFFER_M", 150.0))
    k_trim = int(endpoint_buffer_m / max(step_m, 1.0e-6))
    if 2 * k_trim < len(ref_xy) - 2:
        ref_xy = ref_xy[k_trim:len(ref_xy) - k_trim]

    # HARD outer corridor around path1: the repulsive field only pushes
    # path2 AWAY from the main, so without a container the very first
    # search simply takes the globally cheapest corridor -- which can be
    # hundreds of meters from a main that was detoured by its TN/FLZ
    # requirements, and identical for both directions (overlapping
    # "backups"). Parallel means near-but-not-touching: soft floor at
    # BUFFER_TARGET_M, hard ceiling at BUFFER_CORRIDOR_MAX_OFFSET_M.
    max_offset_m = float(pget(params, "BUFFER_CORRIDOR_MAX_OFFSET_M", 250.0))
    corridor_mask: np.ndarray | None = None
    if max_offset_m > 0.0:
        xy_all = df[["x", "y"]].to_numpy(float)
        try:
            from scipy.spatial import cKDTree
            d_center, _ = cKDTree(ref_xy_full).query(xy_all, k=1)
        except Exception:
            d_center, _ = _min_distance_to_polyline(xy_all, ref_xy_full)
        corridor_mask = d_center <= max_offset_m

    result = run_project_theta(
        base_model=df, cell_to_idx=cell_to_idx, base_allowed_mask=base_allowed,
        start_idx=start_idx, end_idx=end_idx, params=params, route_name=route_name,
        extra_allowed_mask=corridor_mask,
    )
    if not result.get("success", False):
        result.setdefault("message", "Parallel route: initial Theta* failed.")
        return result

    path2 = [int(v) for v in result.get("path_indices", [])]
    best_path2 = path2
    best_result = result
    ...
    for it in range(max_iter):
        ...
        sample_xy, dist, side = compute_clearance_profile(ref_xy, _route_xy(df, path2))
        crossings = detect_crossing_points(sample_xy, dist, side)
        score = parallel_violation_score(dist, len(crossings), params)
        ...
        if score < best_score - 1.0e-9:
            ...
            # No improvement over best-so-far (the search may oscillate
            # between a few variants without ever repeating the immediately
            # previous path): escalate the penalty per the design flowchart.
            lam *= growth
            if lam > lam_cap:
                break  # accept the best seen so far, log remaining violations

        if not (dist < target).any() and len(crossings) == 0:
            break  # soft buffer fully satisfied, stop early

        df_pen = build_repulsive_slowness_model(df, ref_xy, lam, crossings, params)
        result_new = run_project_theta(
            base_model=df_pen, cell_to_idx=cell_to_idx, base_allowed_mask=base_allowed,
            start_idx=start_idx, end_idx=end_idx, params=params,
            route_name=f"{route_name}_lam{lam:.1f}",
            extra_allowed_mask=corridor_mask,
        )
        path2_new = [int(v) for v in result_new.get("path_indices", [])] if result_new.get("success", False) else []

        if not path2_new or path2_new == path2:
            # Identical result at this lambda: escalate without wasting
            # another search on the same field.
            lam *= growth
            if lam > lam_cap:
                break
        else:
    ...
    # Final violation report against the BEST path2 (a late, over-penalized
    # iteration can be worse than an earlier one).
    sample_xy, dist, side = compute_clearance_profile(ref_xy, _route_xy(df, best_path2))
    crossings = detect_crossing_points(sample_xy, dist, side)
    narrow = dist < target

    out = dict(best_result)
    out["path_indices"] = best_path2
    out["distance_m"] = path_distance_m(df, best_path2)
    out["duplicated_from_main"] = bool(best_path2 == [int(v) for v in main_path])
    out.update(backup_vs_main_geometry(df, main_path, best_path2))
    out["parallel_buffer_ok"] = bool(not narrow.any() and len(crossings) == 0)
    out["parallel_min_clearance_m"] = float(dist.min()) if len(dist) else float("nan")
    out["parallel_n_narrow_segments"] = int(narrow.sum())
    out["parallel_n_crossings"] = int(len(crossings))
    out["parallel_crossing_locations"] = ";".join(f"({p[0]:.0f},{p[1]:.0f})" for p in crossings)
    out["parallel_final_lambda"] = float(lam)
    out["parallel_iterations"] = int(iterations_used)
    if not out["parallel_buffer_ok"]:
        out["message"] = (
            str(out.get("message", "")).strip()
            + f" Soft buffer: {int(narrow.sum())} narrow sample(s), {len(crossings)} crossing(s), "
            + f"min clearance {float(dist.min()):.1f} m (target {target:.0f} m)."
        ).strip()
    return out
...
def plot_tn_nodes_with_buffer(
    ...
    """Draw TN major/minor candidate markers, plus a translucent ring at
    each node showing its own buffer radius (NEW in 06 -- without this,
    the hard TN buffer requirement is invisible in the figures)."""
    if "tn_role" not in df.columns:
        return
    tn_on, major_r, minor_r = tn_buffer_settings(params)
    show_rings = bool(pget(params, "PLOT_SHOW_TN_BUFFER", True)) and tn_on
    tn_style = {
        "major": ("D", "darkcyan", 90, major_r),
        "minor": ("d", "mediumturquoise", 50, minor_r),
    }
    for role, (marker, color, size, radius) in tn_style.items():
        sub = df[df["tn_role"] == role]
        if len(sub) == 0:
            continue
        ax.scatter(sub["x"], sub["y"], marker=marker, s=size, c=color, edgecolors="white", linewidths=0.8, zorder=9, label=f"TN_{role}")
        for _, row in sub.iterrows():
            label = str(row.get("candidate_id", "")) or str(row.get("node_id", ""))
            ax.text(row["x"], row["y"], label, fontsize=max(text_size - 2, 5), color=color, weight="bold", zorder=11, fontfamily=font_family)
            if show_rings and radius > 0:
                ax.add_patch(plt.Circle((row["x"], row["y"]), radius, fill=False, edgecolor=color, linewidth=0.6, alpha=0.5, zorder=4))
...
def plot_pair_routes(
    ...
    plot_tn_nodes_with_buffer(ax, df, params, font_family, text_size)
...
def plot_overview(
    ...
    ax.set_title("All Theta* master corridor routes", fontfamily=font_family, fontsize=title_size, fontweight="bold")
    ax.set_xlabel("X coordinate (m)", fontfamily=font_family, fontsize=label_size)
    ax.set_ylabel("Y coordinate (m)", fontfamily=font_family, fontsize=label_size)
    ax.set_aspect("equal", adjustable="box")
    add_map_rule(ax)
...
def process_single_pair(
    ...
    cross_pair_locked_paths (NEW in 06): OTHER pairs' already-accepted
    routes, read-only avoid-list for this pair's cross-pair overlap check.
    Only meaningful under sequential processing (CROSS_PAIR_OVERLAP_ENABLE
    forces N_CORES=1 in main()) since it requires each pair to see every
    earlier pair's result before planning its own routes.
    ...
    with contextlib.redirect_stdout(log_buf):
        ...
        if direction_order == "interleaved":
            ...
            for rank in range(1, int(n_route_sets) + 1):
                forward_routes.update(generate_direction_route_sets(
                    ...
                    cross_pair_locked_paths=cross_pair_locked_paths,
        ...
        for route_key, result in routes.items():
            ...
            row = {
                ...
                "parallel_buffer_ok": result.get("parallel_buffer_ok", np.nan),
                "parallel_min_clearance_m": result.get("parallel_min_clearance_m", np.nan),
                "parallel_n_narrow_segments": result.get("parallel_n_narrow_segments", np.nan),
                "parallel_n_crossings": result.get("parallel_n_crossings", np.nan),
                "parallel_crossing_locations": result.get("parallel_crossing_locations", ""),
                "parallel_final_lambda": result.get("parallel_final_lambda", np.nan),
                "parallel_iterations": result.get("parallel_iterations", np.nan),
                ...
                "tn_buffer_ok": result.get("tn_buffer_ok", np.nan),
                "tn_buffer_min_distance_m": result.get("tn_buffer_min_distance_m", np.nan),
                "tn_buffer_matched_role": result.get("tn_buffer_matched_role", ""),
                "tn_buffer_via": result.get("tn_buffer_via", ""),
                "n_waypoints_before_turn_reduction": result.get("n_waypoints_before", np.nan),
                "n_waypoints_after_turn_reduction": result.get("n_waypoints_after", np.nan),
                "max_turn_angle_deg_before": result.get("max_turn_angle_deg_before", np.nan),
                "max_turn_angle_deg_after": result.get("max_turn_angle_deg_after", np.nan),
                "mean_turn_angle_deg_before": result.get("mean_turn_angle_deg_before", np.nan),
                "mean_turn_angle_deg_after": result.get("mean_turn_angle_deg_after", np.nan),
                "turn_reduction_applied": result.get("turn_reduction_applied", False),
                "cross_pair_overlap_ok": result.get("cross_pair_overlap_ok", np.nan),
                "cross_pair_overlap_percent": result.get("cross_pair_overlap_percent", np.nan),
                "cross_pair_overlap_nodes": result.get("cross_pair_overlap_nodes", np.nan),
        ...
        if bool(pget(params, "SAVE_PAIR_FIGURES", True)):
            plot_pair_routes(
                ...
                routes=routes,
                pair_name=pair_name,
                output_file=figure_dir / f"{pair_name}_theta_routes_v1.png",
                nofly_threshold=nofly_threshold,
...
def main() -> None:
    ...
    cp_on, _ = cross_pair_overlap_settings(params)
    if cp_on:
        # Cross-pair overlap (NEW in 06) needs each pair to see every
        # EARLIER pair's accepted routes before planning its own -- that
        # shared, growing state can't safely cross OS process boundaries
        # the way ProcessPoolExecutor's independent pairs do, so force
        # sequential regardless of N_CORES.
        n_cores_param = 1
        n_cores = 1
    ...
    if n_cores <= 1:
        ...
        for pair_id, pair in enumerate(pairs, start=1):
            res = process_single_pair(
                ...
                cross_pair_locked_paths=list(cross_pair_locked_paths) if cp_on else None,
            )
            ...
            if cp_on:
                cross_pair_locked_paths.extend(res.get("accepted_paths", []))
    else:
        worker_args = [
            (
                pair_id, pair, df, cell_to_idx, base_allowed, params,
    ...
    summary_file = output_dir / "route_summary_v1.csv"
    ...
    edge_file = output_dir / "all_route_edges_v1.csv"
    ...
    if bool(pget(params, "SAVE_OVERVIEW_FIGURE", True)):
        ...
        overview_figures = figure_dir / "00_all_theta_routes_overview_v1.png"
        overview_parent = output_dir / "00_all_theta_routes_overview_v1.png"
