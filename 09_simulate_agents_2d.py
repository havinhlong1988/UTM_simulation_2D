...
def simulate(net: Network, agents: list[Agent], patrols: list[Agent], params):
    ...
    def enter_leg(a: Agent, idx: int) -> bool:
        ...
        if flow_block:
            ...
            if entry < merge_gap:
                return False
        if node_mutex and a.held_node != seg.src_node:
            h = node_busy.get(seg.src_node)
            if h is not None and h is not a:
                return False
            node_busy[seg.src_node] = a
            a.held_node = seg.src_node
        if flow_block:
            leg_busy[seg.res] = a
        else:
    ...
    while t <= horizon:
        ...
        if n_waiting and n_active < max_concurrent:
        ...
        for a in movers:
            ...
            if node_mutex:
                ...
                if a.held_node == seg.src_node and \
                        max(a.s_local, min(desired, L)) > zone_src:
                    release_node(a)
                approach_s = L - zone_dst
                if desired > approach_s and a.held_node != seg.dst_node:
                    h = node_busy.get(seg.dst_node)
                    if h is None or h is a:
                        node_busy[seg.dst_node] = a; a.held_node = seg.dst_node
                    else:
        ...
        for a in active_agents:
            if a.status == "dwell":
                if a.battery_wh < charge_target - 1e-6:     # still charging
                    a.battery_wh = min(battery_cap, a.battery_wh + charge_power * dt / 3600.0)
                    a.charge_s += dt
                elif enter_leg(a, a.seg_idx + 1):           # charged: depart when lane free
                    moved = True
                # else: parked at dock, waiting for a free lane (no hover drain)
        ...
        if t >= next_sample - 1e-9:
            ...
            if len(dxy) >= 2:
                ...
                dij = d[iu]
                approach = float(dij.min())
                min_approach_global = min(min_approach_global, approach)
                thr = (conflict_time * np.maximum(dv[:, None], dv[None, :]))[iu]
                # opposite lanes of the SAME corridor are spatially separated
                # by design -> not a conflict; exclude same-leg pairs
            ...
            frames.append((t, xy, outb, hold, aids, spd, conflict_pts))
...
def make_animation(net: Network, frames, params, sep_eff, out_gif: Path):
    ...
    def update(k):
        t, xy, outb, hold, _aids, spd, cf = frames[k]
        if len(xy):
            si = sidx_of(spd)
            for d in (1, 0):
                for c in range(len(cls_desc)):
...
def write_html(net: Network, frames, params, sep_eff, out_html: Path):
    ...
    fdata, cdata = [], []
    for (_t, xy, outb, hold, aids, spd, cf) in frames:
        rows = []
...
let k=0, playing=true, base=8, mult=1, acc=0, last=performance.now();

function loop(now){
  const dt=(now-last)/1000; last=now;
  if(playing){
    acc += dt*base*mult;
    while(acc>=1){ acc-=1; k=(k+1)%N; }
    scrub.value=k; draw(k);
  }
  requestAnimationFrame(loop);
}
document.getElementById('play').onclick=e=>{ playing=!playing; e.target.textContent=playing?'Pause':'Play'; last=performance.now(); };
document.getElementById('restart').onclick=()=>{ k=0; scrub.value=0; draw(0); };
document.getElementById('speed').onchange=e=>{ mult=parseFloat(e.target.value); };
scrub.oninput=e=>{ k=parseInt(e.target.value); playing=false;
  document.getElementById('play').textContent='Play'; draw(k); };
const cmtoggle=document.getElementById('cmtoggle');
if(cmtoggle){ cmtoggle.disabled=!DATA.costmap; cmtoggle.checked=showCostmap;
  cmtoggle.onchange=e=>{ showCostmap=e.target.checked; draw(k); }; }
...
def _pv_base_scene(net: Network, params, pl):
    ...
    import pyvista as pv
    z_out = float(pget(params, "PYVISTA_Z_OUTBOUND", 60.0))
    z_in = float(pget(params, "PYVISTA_Z_INBOUND", 100.0))
    x0, y0 = net.nodes["x"].min(), net.nodes["y"].min()
    x1, y1 = net.nodes["x"].max(), net.nodes["y"].max()
    # faint ground plane
    plane = pv.Plane(center=((x0 + x1) / 2, (y0 + y1) / 2, -2.0),
    ...
    # translucent operation-plane sheets at the two cruise altitudes
    for z, c in ((z_out, "#2f6fed"), (z_in, "#e23b3b")):
        sheet = pv.Plane(center=((x0 + x1) / 2, (y0 + y1) / 2, z),
                         i_size=(x1 - x0) * 1.05, j_size=(y1 - y0) * 1.05)
        pl.add_mesh(sheet, color=c, opacity=0.05, show_edges=False)
...
def _agent_points(frame, z_out, z_in, z_patrol):
    """(out_xyz, in_xyz, patrol_xyz, hold_xyz) for one frame, lifted to
    per-category cruise planes (outbound low, inbound high, patrol top)."""
    _t, xy, outb, hold, _a, _spd, _cf = frame
    out_p, in_p, pat_p, hold_p = [], [], [], []
    for i in range(len(xy)):
        cat = int(outb[i])
        z = z_out if cat == 1 else (z_patrol if cat == 2 else z_in)
...
def render_pyvista(net: Network, frames, params, sep_eff, out_dir: Path):
    ...
    z_out = float(pget(params, "PYVISTA_Z_OUTBOUND", 60.0))
    z_in = float(pget(params, "PYVISTA_Z_INBOUND", 100.0))
    z_patrol = float(pget(params, "PYVISTA_Z_PATROL", 140.0))
    ...
    def draw_agents(pl, frame):
        out_p, in_p, pat_p, hold_p = _agent_points(frame, z_out, z_in, z_patrol)
...
def write_trajectories(frames, out_csv: Path):
    ...
    rows = []
    for (t, xy, outb, hold, aids, spd, _cf) in frames:
        for i in range(len(xy)):
